...
pdf("IndivLociOutputs/Siler_b2homCAT.pdf")

## set up inbreeding loci
inb <- CH$het
inb <- 1 - rescale(inb) # now a measure of homozygosity
median(inb)
inb <- if_else(inb >= median(inb), 1, 0)
...
initFn <- function(cint, censored, inb) {
  ...
  optFn <- function(pars, t, inb) {
    ...
    ll_ho <- sum(dSiler(t[inb == 1], a1 = pars[1], a2 = pars[2], b1 = pars[3], 
                        b2 = pars[4] * exp(pars[6]), c1 = pars[5], log = TRUE))
    
    ll_he <- sum(dSiler(t[inb == 0], a1 = pars[1], a2 = pars[2], b1 = pars[3], 
...
## save MCMC
saveRDS(samples, "IndivLociOutputs/samples_s_b2hetCAT.rds")
#samples <- readRDS("IndivLociOutputs/samples_s_b2L22.rds")
...
saveRDS(logimpweight, "IndivLociOutputs/logimpweight_s_b2hetCAT.rds")
