...
code <- nimbleCode({
  ...
  for (i in 1:nind) {
    ...
    log(b2mult[i]) <- betaINBR * inbr[i] * z
    #beta[7] * sex[i]  +
...
## save mcmc ouput
saveRDS(run, "outputs/m1_InbrOnSepParams/m1_InbrB2_runsamples.rds")
...
res
res
saveRDS(res, "outputs/m1_InbrOnSepParams/m1_InbrB2_PosteriorModelProbs.rds")
