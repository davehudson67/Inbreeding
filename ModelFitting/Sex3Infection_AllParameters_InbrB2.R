...
code <- nimbleCode({
  ...
  for (i in 1:nind) {
    ...
    ## likelihood for interval-truncated siler
      ...
      tD[i] ~ dsilerNim(a1 * a1mult[i], a2 * a2mult[i], b1 * b1mult[i], b2 * b2mult[i], c1 * c1mult[i])
          ...
          log(a1mult[i]) <- beta[1] * sex[i] + 
                            beta[2] + (betaINFCUB[1] * infection[i, 3] + betaINFADULT[1] * infection[i, 2]) 
            
          log(a2mult[i]) <- beta[3] * sex[i] + 
                            beta[4] + (betaINFCUB[2] * infection[i, 3] + betaINFADULT[2] * infection[i, 2]) 
            
          log(b1mult[i]) <- beta[5] + (betaINFCUB[3] * infection[i, 3] + betaINFADULT[3] * infection[i, 2]) 
            
          log(b2mult[i]) <- betaINBR[1] * inbr[i] * z[1] +
                            betaINBR[2] * sex[i] * z[2] +
                            betaINBR[3] + (betaINFCUB[4] * infection[i, 3] + betaINFADULT[4] * infection[i, 2]) * z[3] +
                            betaINBR[4] * inbr[i] * sex[i] * z[4] +
                            betaINBR[5] * inbr[i] * (betaInbrInfCUB * infection[i, 3] + betaInbrInfADULT * infection[i, 2]) * z[5]
                            
          log(c1mult[i]) <- beta[6] * sex[i] + 
                            beta[7] + (betaINFCUB[5] * infection[i, 3] + betaINFADULT[5] * infection[i, 2]) 
  ...
  for (j in 1:5){
    ...
    z[j] ~ dbern(0.5)
    betaINFCUB[j] ~ dnorm(0, sd = 1)
    betaINFADULT[j] ~ dnorm(0, sd = 1)
  }
  ...
  b1 ~ dexp(1)
  b2 ~ dexp(1)
  betaInbrInfCUB ~ dnorm(0, sd = 1)
  betaInbrInfADULT ~ dnorm(0, sd = 1)
  c1 ~ dexp(1)
  mean.p ~ dunif(0, 1)
  constraint_dataSexInb1 ~ dconstraint(z[4] <= z[1] * z[2])
  constraint_dataInfInb1 ~ dconstraint(z[5] <= z[1] * z[3])
...
initFn <- function(cint, censored) {
  ...
  list(
    ...
    betaInbrInfCUB = rnorm(1, 0, 1),
    betaInbrInfADULT = rnorm(1, 0, 1),
    z = rep(0, times = 5)
  )
}
...
config$addSampler(target = c("b2"), type = 'slice', control = list(sliceWidth = 0.2, adaptInterval = 20))
config$addSampler(target = c("c1"), type = 'slice', control = list(sliceWidth = 0.5, adaptInterval = 50))
config$addSampler(target = c("betaINFCUB", "betaINFADULT"), type = 'AF_slice', control = list(sliceAdaptFactorInterval = 100))
...
## save mcmc ouput
saveRDS(run, "outputs/Sex3Infection_AllParameters_InbrB2runsamples.rds")
#run <- readRDS("outputs/Sex3Infection_AllParameters_InbrB2runsamples.rds")
...
res
res
saveRDS(res, "outputs/Sex3Infection_AllParameters_InbrB2PosteriorModelProbs.rds")
...
z_indicators$parameter <- c("Inbreeding_b2", "Sex_b2", "Infection_b2", 
                            "Sex:Inbreeding_b2", "Infection:Inbreeding_b2")
