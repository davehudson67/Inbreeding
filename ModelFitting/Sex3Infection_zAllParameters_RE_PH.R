...
ggplot(dt_inbr, aes(y = dt, x = inbr, colour = sex)) +
  geom_point() + 
  labs(y = "inferred death age - 1/4 yrs", x = "MLHomozygosity") +
...
dt_inbr %>%
  filter(dt > 32) %>%
  ggplot() +
  geom_point(aes(y = dt, x = inbr, colour = sex)) + 
  labs(y = "inferred death age - 1/4 yrs", x = "MLHomozygosity")
