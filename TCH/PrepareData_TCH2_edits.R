...
CH <- select(CH, tattoo, date, pm, occ, capture_yr, sex, age, year_fc, age_fc_yrs, trap_season, statpak, GAMMA, Cult_Sum, brock, v1s)
...
## change statpak
CH <- CH %>%
  mutate(statpak = if_else(statpak == "FALSE", 0, 1)) %>%
  mutate(Cult_Sum = if_else(Cult_Sum > 1, 1, 0))

## set up number of tests delivered on each capture
CH$Num_tests <- NA
for (i in 1:nrow(CH)) {
  CH$Num_tests[i] <- length(which(!is.na(CH[i,11:15])))
}
CH$Pos_tests <- NA
for (i in 1:nrow(CH)) {
  CH$Pos_tests[i] <- sum(CH[i, 11:15], na.rm = TRUE)
}

## create probabilistic measure of infection
CH$Pos <- CH$Pos_tests / CH$Num_tests

## confirm if culture = 1 then individual is infected
CH$Pos <- if_else(CH$Cult_Sum == 1, 1, CH$Pos, missing = CH$Pos)

## decide on positive definition
CH$PosCAT <- ifelse(CH$Pos < 0.3, 0, 1)
...
CH <- CH %>%
  group_by(ID) %>%
  mutate(infected_now = ifelse(statpak == 1 | IFNgamma == 1 | Cult_Sum > 0 | brock == 1 | v1s == 1, 1, 0)) %>%
  mutate(infected2 = ifelse(sum(statpak[statpak > 0], IFNgamma[IFNgamma > 0], Cult_Sum[Cult_Sum > 0], brock[brock > 0], v1s[v1s > 0]) > 1, 1, 0)) %>%
 ...
 # mutate(infected_lifetime = max(infected)) %>%
  ...
  mutate(infected_as_cub_Pos = ifelse(Pos == 1 & age <= 1, 1, 0)) %>%
 # mutate(infected_as_cub = max(infected_as_cub)) %>%
  ungroup()
...
CH$infected_as_cub_Pos <- as.factor(CH$infected_as_cub_Pos)
