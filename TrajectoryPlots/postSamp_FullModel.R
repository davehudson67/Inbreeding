...
post <- sample_n(as.data.frame(post), 500000)
post <- as.matrix(post)
...
newdata <- expand.grid(t = 0:80, sex = 0, infection1 = 0, infection2 = 1, inbr = 1)
...
for(i in 1:5) {
  ...
  zpost <- post[, match(paste0("z", c("INBR", "SEXINBR", "INFINBR", "SEXINF"), "[", i, "]"), colnames(post))]
...
saveRDS(postSurv, "outputs/PostSurv_FullModel_zAll.rds")
saveRDS(postMort, "outputs/PostMort_FullModel_zAll.rds")


ggplot(postMort, aes(x = t)) +
  geom_line(aes(y = Median, colour = MLH)) +
#  geom_ribbon(aes(ymin = LCI, ymax = UCI, fill = MLH), alpha = 0.5) +
