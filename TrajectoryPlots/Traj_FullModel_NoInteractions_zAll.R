...
post <- readRDS("outputs/FullModel_InbrCont_NoInteractions_z_all_runsamples.rds")
post <- as.matrix(post$samples)
#post <- sample_n(as.data.frame(post), 100000)
post <- as.matrix(post)
...
for(i in 1:5) {
  ...
  parPost <- post[, match(paste0("beta", c("SEX", "INFCUB", "INFADULT", "INBR"), "[", i, "]"), colnames(post))]
  
  ## extract inclusion samples
  zpost <- post[, match(paste0("z", c("INBR", "INF", "SEX"), "[", i, "]"), colnames(post))]
...
saveRDS(postSurv, "outputs/PostSurv_FullModel_NoInteractions_zAll.rds")
saveRDS(postMort, "outputs/PostMort_FullModel_NoInteractions_zAll.rds")
