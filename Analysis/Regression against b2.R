...
initFn <- function(cint, censored, inbreeding) {
  ...
  optFn <- function(pars, t) {
    ...
    sum(dSiler(t, a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = (pars[4] + (pars[5] * inbreeding)), c = pars[5], log = TRUE))
  }
...
levels(CH.all$status)
levels(CH.all$status.cub)
CH <- CH.all[CH.all$status == 'NPF' | CH.all$status == 'NPM'] #positive badgers
#CH <- CH.all[CH.all$status.cub == 'CPF' | CH.all$status.cub == 'CPM'] #cub positive
#CH <- CH.all[CH.all$status == 'NPF' | CH.all$status.cub == 'NPM'] #never positive
inbreed <- CH$f_inbreed
